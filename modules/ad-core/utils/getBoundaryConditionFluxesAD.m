...
isCompositional = isa(model, 'ThreePhaseCompositionalModel') || isa(model, 'ECPAThreePhaseCompositionalModel');
...
singleWater = strcmp(cnames{1}, longNames{1});
rhoAvgF = cell(nph, 1);
for i = 1:nph
    % Wei edit this part
    if isCompositional && i > singleWater %model.water    % is water a component£¿
        if isTransport
            sL = sF{i, 1}./sT{1};
            sR = sF{i, 2}./sT{2};
        else
