...
% --------------------------------------------------------------------%
    ...
    function [eqs, state, src] = addBoundaryConditionsAndSources(model, eqs, names, types, state, ...
        ...
        for i = 1:numel(s)
            % Wei edit this part
            sub = strcmp(names, longNames{i}); %strcmpi(names, longNames{i}); % is water a single phase?
            if any(sub)
                assert(strcmpi(types{sub}, 'cell'), 'Unable to add source terms to equation that is not per cell.');
        ...
        for i = 1:numel(cnames)
            ...
            name = cnames{i};
            if any(strcmp(longNames, name))
                % Component corresponds to pseudocomponent and has already
                % been dealt with
                continue
            end
            sub = strcmp(name, names); % strcmpi(name, names);
            if any(sub)
                eq = eqs{sub};
            else
            ...
            [eq, bnd_cond] = model.addComponentContributions(name, eq, C, bnd_cond, forces.bc, rho);  % Wei add rho
            [eq, src_terms] = model.addComponentContributions(name, eq, C, src_terms, forces.src, rho);% Wei add rho
            if any(sub)
                eqs{sub} = eq;
            end
        end
