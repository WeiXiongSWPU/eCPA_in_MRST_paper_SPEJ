            ...
            for i = 1:2
                ...
                if i == 2 && prop.useCompactEvaluation && ~all(twoPhase)
                    ...
                    if any(twoPhase)
                        ...
                        if iscell(m.a)
                            a = cellfun(@(x) x(twoPhase), m.a, 'UniformOutput', false);
                        else
                            a = a(twoPhase, :);
                        end
                        if iscell(m.acti)
                            acti = cellfun(@(x) x(twoPhase), m.acti, 'UniformOutput', false);
                        else
                            acti = acti(twoPhase, :);
                        end
                        if iscell(m.Tr)
                        ...
                        f_2ph = model.EOSModel.computeFugacity(T(twoPhase),p(twoPhase),xy,m.A(twoPhase), m.B(twoPhase),a, z.v(twoPhase),acti, XA, z.XC(twoPhase), Tr);
                        for j = 1:numel(fi)
                    ...
                    f(:, i) = model.EOSModel.computeFugacity(T, p, xy, m.A, m.B, m.a, z.v, m.acti, z.XA, z.XC, m.Tr);
                end
            end
        end
    end
