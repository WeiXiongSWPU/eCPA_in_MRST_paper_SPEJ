classdef ECPAPhaseCompressibilityFactorsLV < StateFunction
    ...
    methods
        ...
        function v = evaluateOnDomain(prop, model, state)
            ...
            [~, v_L, XA_L, XC_L] = eos.computeCompressibilityZ(p, T, x, L_mix.A, L_mix.B, L_mix.a, L_mix.acti, L_mix.Tr, true);
            ...
            if prop.useCompactEvaluation && ~all(twoPhase)
                ...
                if any(twoPhase)
                    ...
                    if isDiagonal
                        ...
                        if iscell(V_mix.acti)
                            acti = cellfun(@(x) x(twoPhase), V_mix.acti, 'UniformOutput', false);
                            Tr = cellfun(@(x) x(twoPhase), V_mix.Tr, 'UniformOutput', false);
                            a = cellfun(@(x) x(twoPhase), V_mix.a, 'UniformOutput', false);
                        else
                            acti = V_mix.acti(twoPhase, :);
                            Tr = V_mix.Tr(twoPhase, :);
                            a = V_mix.a(twoPhase, :);
                        end
                        A = V_mix.A(twoPhase); 
                        B = V_mix.B(twoPhase); 
                        [~, v_V(twoPhase), XA2_V, XC_V(twoPhase)] = eos.computeCompressibilityZ(p(twoPhase), T(twoPhase), y, A, B, a, acti, Tr, false);
                        ...
                        if iscell(V_mix.acti)
                            acti = cellfun(@(x) x(twoPhase), V_mix.acti, 'UniformOutput', false);
                            acti = cellfun(@(x) prop.dpADI(x, 1, twoPhase), acti, 'UniformOutput', false);
                            Tr = cellfun(@(x) x(twoPhase), V_mix.Tr, 'UniformOutput', false);
                            a = cellfun(@(x) x(twoPhase), V_mix.a, 'UniformOutput', false);
                        else
                            acti = V_mix.acti(twoPhase, :);
                        ...
                        [~, v_V2, XA2_V, XC_V2] = eos.computeCompressibilityZ(p, T, y, A, B, a, acti, Tr, false);
                ...
                [~, v_V, XA_V, XC_V] = eos.computeCompressibilityZ(p, T, y, V_mix.A, V_mix.B, V_mix.a, V_mix.acti, V_mix.Tr, false);
