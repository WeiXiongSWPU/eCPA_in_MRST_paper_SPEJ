...
function [xy, S, trivialSolution, K] = checkStability(eos, z, K, p, T, insidePhaseIsVapor, active, opt)
    ...
    for stepNo = 1:opt.maxIterations
        ...
        [f_xy, ~] = getFugacity(eos, T_loc, xy_loc, p_loc, ~insidePhaseIsVapor);
        if insidePhaseIsVapor
...
function [f, v] = getFugacity(model, T, xy, p, isLiquid)
    [bic, a, Tr] = model.getBic(T, iscell(xy));
    [A, B, acti] = model.getPhaseMixCoefficients(xy, T, bic, a);
    [~, v, XA, XC] = model.computeCompressibilityZ(p, T, xy, A, B, a, acti, Tr, isLiquid);
    f = model.computeFugacity(T, p, xy, A, B, a, v, acti, XA, XC, Tr);
end