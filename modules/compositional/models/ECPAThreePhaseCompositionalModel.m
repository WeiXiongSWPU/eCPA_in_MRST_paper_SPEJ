classdef ECPAThreePhaseCompositionalModel < ReservoirModel
    ...
    methods
        function model = ECPAThreePhaseCompositionalModel(G, rock, fluid, compFluid, varargin)
            model = model@ReservoirModel(G, rock, fluid);
            
            if isa(compFluid, 'ECPACompositionalMixture')
                model.EOSModel = ECPAEquationOfStateModel(G, compFluid);
            elseif isa(compFluid, 'ECPAEquationOfStateModel')
            ...
            if ~isempty(report)
                ...
                if ~report.StepReports{1}.Converged
                    ...
                    for i = 1:numel(report.StepReports{1}.NonlinearReport{end}.Residuals)
                        fprintf('* %s: %1.4g \n', model.EOSModel.ECPACompositionalMixture.names{i},....
        ...
        function [eq, src] = addComponentContributions(model, cname, eq, component, src, force, rho)
            if isempty(force)
                return
            end
            ...
            if any(sub)
                ...
                if isfield(force, 'componentMass')
                    ...
                    if isfield(model.rock, 'Mechanisms') && isfield(model.rock.Mechanisms,'diffusion')
                        % Wei add diffusion
                        G = model.G;
                        C = G.faces.centroids(force.face, :) - G.cells.centroids(cells,:);
                        sgn = 2*(cells == G.faces.neighbors(force.face, 1)) - 1;
                        N   = bsxfun(@times, sgn, G.faces.normals(force.face, :));
                        [~, i, j] = permTensor(model.rock, G.griddim);
                        % Compute T = C'*N / C'*C.
                        T = zeros(size(cells));
                        for k = 1 : size(i, 2)
                            T = T + (C(:, i(k)) .* N(:, j(k)));
                        ...
                        cellToBCMap = sparse((1:nbc)', cells, 1, nbc, G.cells.num);
                        ...
                        for i = 1:nph
                            ...
                            if isfield(force, 'rho')
                                rhoBC_bnd = force.rho(:, i);
                            else
                        ...
                        diffusionFlux = - model.rock.Di(1, sub) ...
                            .* model.rock.poro(cells) ...
                            .* sat(:,1) .* T ./ model.rock.tau ...
                            .* (massFractions{1}(:, sub) .* rhoF{1, 1} ...
                            - component{1}(cells) .* rhoF{1, 2});
                        qC = qC - diffusionFlux;
                        src.diffusionFlux{end+1} = {value(diffusionFlux)};
                    end
                end
        ...
        function model = setupStateFunctionGroupings(model, varargin)
            ...
            pvtprops = pvtprops.setStateFunction('Density', ECPACompositionalDensity(model));
            ...
            pvtprops = pvtprops.setStateFunction('PhaseMixingCoefficients', ECPAPhaseMixingCoefficientsLV(model));
            pvtprops = pvtprops.setStateFunction('Fugacity', ECPAFugacityLV(model));
            pvtprops = pvtprops.setStateFunction('PhaseCompressibilityFactors', ECPAPhaseCompressibilityFactorsLV(model));
            pvtprops = pvtprops.setStateFunction('Viscosity', ECPACompositionalViscosityLV(model));
