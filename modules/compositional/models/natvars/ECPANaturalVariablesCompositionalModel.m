classdef ECPANaturalVariablesCompositionalModel < ECPAThreePhaseCompositionalModel
    ...
    methods
        function model = ECPANaturalVariablesCompositionalModel(G, rock, fluid, mixture, varargin)
            model = model@ECPAThreePhaseCompositionalModel(G, rock, fluid, mixture, varargin{:});
        end
        
        function [problem, state] = getEquations(model, state0, state, dt, drivingForces, varargin)
            [problem, state] = eCPAequationsNaturalVariables(state0, state, model, dt, ...
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            [state, report] = updateState@ECPAThreePhaseCompositionalModel(model, state, problem, dx, drivingForces);
        ...
        function [convergence, values, names] = checkConvergence(model, problem, varargin)
            [convergence, values, names] = checkConvergence@ECPAThreePhaseCompositionalModel(model, problem, varargin{:});
        ...
        function state = validateState(model, state)
            state = validateState@ECPAThreePhaseCompositionalModel(model, state);
