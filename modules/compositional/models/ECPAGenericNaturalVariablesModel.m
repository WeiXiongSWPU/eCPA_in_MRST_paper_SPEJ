classdef ECPAGenericNaturalVariablesModel < ECPANaturalVariablesCompositionalModel & GenericReservoirModel
    properties
        
    end
    
    methods
        function model = ECPAGenericNaturalVariablesModel(varargin)
            model = model@ECPANaturalVariablesCompositionalModel(varargin{:});
        ...
        function [eqs, names, types, state] = getModelEquations(model, state0, state, dt, drivingForces)
            ...
            [eqs, ~, bcs] = model.addBoundaryConditionsAndSources(eqs, names, types, state, ...
                                                             pressures, sat, mob, rho, ...
                                                             {}, comps, ...
                                                             drivingForces);
            state.diffusionFlux = bcs.bc.diffusionFlux;
            % Add sources
            eqs = model.insertSources(eqs, src);
            
            % Assemble equations
            if isfield(model.rock, 'Mechanisms') && isfield(model.rock.Mechanisms,'diffusion')
                xM = X(:, model.getLiquidIndex);
                sO = sat{model.getLiquidIndex};
                flag = model.getProps(state, 'PhaseUpwindFlag');
                upco = flag{model.getLiquidIndex};
                rhoO = rho{model.getLiquidIndex};
            end
            for i = 1:numel(eqs)
                eqs{i} = model.operators.AccDiv(eqs{i}, flux{i});
                if isfield(model.rock, 'Mechanisms') && isfield(model.rock.Mechanisms,'diffusion')
                    % add diffusion:
                    So_poro = model.operators.faceUpstr(upco,sO.*model.rock.poro);
                    Jo = model.operators.Div(model.operators.T_diff{i}./model.rock.tau.*So_poro.*model.operators.Grad(xM{i}.*rhoO));
            ...
            for i = 1:n_hc
                if any(twoPhase)
                    nmole = model.EOSModel.getNumberOfMolecules();
                    if i<=nmole
                        f_eqs{i} = (f{i, 1}(twoPhase) - f{i, 2}(twoPhase))/barsa;
                    else
                        scale = value(f{i, 2}(twoPhase))./value(f{i, 1}(twoPhase));
                        f_eqs{i} = (f{i, 1}(twoPhase).*scale - f{i, 2}(twoPhase))/barsa;
                    end
                end
        ...
        function [state, report] = updateState(model, state, problem, dx, forces)
            [state, report] = updateState@ECPANaturalVariablesCompositionalModel(model, state, problem, dx, forces);
        ...
        function model = validateModel(model, varargin)
            ...
            model = validateModel@ECPANaturalVariablesCompositionalModel(model, varargin{:});
        end
        
        function model = setupStateFunctionGroupings(model, varargin)
            model = setupStateFunctionGroupings@ECPANaturalVariablesCompositionalModel(model, varargin{:});
            model.FlowDiscretization.GravityPotentialDifference.saturationWeighting = true;
        end
        
        function [state, report] = updateAfterConvergence(model, state0, state, dt, drivingForces)
            [state, report] = updateAfterConvergence@ECPAThreePhaseCompositionalModel(model, state0, state, dt, drivingForces);
        ...
        function forces = validateDrivingForces(model, forces, varargin)
            forces = validateDrivingForces@ECPANaturalVariablesCompositionalModel(model, forces, varargin{:});
