classdef ECPAEquationOfStateModel < PhysicalModel
    ...
    methods
        ...
        function [Z, v_m, XA, XC] = computeCompressibilityZ(model, p, T, xy, A, B, a, acti, Tr, isLiquid)
            if iscell(xy)
                xy = cellfun(@value, xy, 'UniformOutput', false);
                xy = [xy{:}];
            end
            if iscell(a)
                a = cellfun(@value, a, 'UniformOutput', false);
                a = [a{:}];
            end
            if iscell(acti)
                acti = cellfun(@value, acti, 'UniformOutput', false);
                acti = [acti{:}];
            end
            if iscell(Tr)
            ...
            for i = 1:1000
                if ~isempty(mu0)
                    [XA_loc, XC_loc, gxy] = model.pressureA(T_loc, xy_loc(:,1:nmole), B_loc, epsilonAB, betaAB, b, v_m_loc, Tr_loc);
                    assoic = 2 .* xy_loc(:,1) .* (2 - XA_loc(:,1) - XC_loc(:,1)) ...
                        + sum(xy_loc(:,2:nmole) .* (1 - XA_loc(:,2:nmole)), 2);
                    assoic = 0.5 .* R .* T_loc .* gxy .* assoic;
                    if ~isempty(d)
                        IFP = model.infiniteFrequencyPermittivity(xy_loc, alpha0, v_m_loc);
                        g = 3.22 - 0.94 .* Tr_loc(:,1);
                        theta = model.pressureAion(xy_loc, hyd);
                        SP = model.staticPermittivity(T_loc, xy_loc, theta, g, mu0, v_m_loc, IFP);
                        Kappa = model.debyeLength(T_loc, xy_loc, charge, v_m_loc, SP);
                        ...
                        PB = model.pressureB(xy_loc(:,nmole+1:end), charge(nmole+1:end), SP, DSPV, v_m_loc, rBorn);
                    else
                        PDH = 0; PB = 0;
                    end
                else
                ...
                if model.selectGibbsMinimum
                    ...
                    if any(multiple)
                        ...
                        xi = xy_loc(multiple, :);
                        [~, phi] = model.computeFugacity(T_loc(multiple), p_loc(multiple), xi, A_loc(multiple), B_loc(multiple), a_loc(multiple, :), V_max, acti_loc(multiple, :), XA_loc(multiple, :), XC_loc(multiple), Tr_loc(multiple, :));
                        g_max = sum(phi .* xi, 2);
                        [~, phi] = model.computeFugacity(T_loc(multiple), p_loc(multiple), xi, A_loc(multiple), B_loc(multiple), a_loc(multiple, :), V_min, acti_loc(multiple, :), XA_loc(multiple, :), XC_loc(multiple), Tr_loc(multiple, :));
                        g_min = sum(phi .* xi, 2);
                        vi = V_max;
                    ...
                    if isLiquid
                        V_m = computeLiquidZ(model, cubic, B_loc);
                    else
                        V_m = computeVaporZ(model, cubic, B_loc);
                    end
                end
                R_norm = sum(abs(v_m_loc-V_m)./v_m_loc,2);
                done = R_norm < 1e-6;
                keep = ~done;
                replace = active;
                replace(active) = done;
        ...
        function [state, report] = stepFunction(model, state, state0, dt, drivingForces, linsolve, nonlinsolve, iteration, varargin) %#ok
            ...
            if iteration == 1
                ...
                t_stability = toc(ts);
                [bic, a, Tr] = model.getBic(T, iscell(x0));
                [A_L, A_V, B_L, B_V, acti_L, acti_V] = model.getMixtureFugacityCoefficients(T, x0, y0, bic, a);
                % Solve EOS for each phase
                Z0_L = model.computeCompressibilityZ(P, T, x0, A_L, B_L, a, acti_L, Tr, true);
                Z0_V = model.computeCompressibilityZ(P, T, y0, A_V, B_V, a, acti_V, Tr, false);
        ...
        function [bic, a, Tr] = getBic(model, T, useCell)
            R = 8.314462618;
            nc = numel(value(T));
            ncomp = model.getNumberOfComponents();
            nmole = model.getNumberOfMolecules();
            ...
            names = model.ECPACompositionalMixture.names;
            ...
            if useCell
                ...
                for i = 1:ncomp
                    if i <= nmole
                        Tr{i} = T.*Ti(i);
                        a{i} = a0(i).*(1+ c1(i).* (1- Tr{i}.^0.5)).^2;
                    else
                        Tr{i} = NaN(nc, 1);
                        a{i} = zeros(nc, 1);
                    end
                end
                kij = model.getMoleculeBIP(Tr, {names{1:nmole}});
                bic = cell(ncomp, ncomp);
                for i = 1 : nmole
                    for j = 1 : nmole
                        if i == j
                            bic{i,j} = zeros(nc,1);
                        else
                            bic{i,j} = log(2).*(a{j}./ b(j)-2.*(a{i}.*a{j}).^0.5 ...
                                ./(b(i)+b(j)).*(1-kij{i,j})) ./ (R .* T);
                        end
                    end
                end
            else
                ...
                kij = model.getMoleculeBIP(Tr, {names{1:nmole}});
                bic = cell(ncomp, ncomp);
                a = bsxfun(@times,a0, (1+bsxfun(@times, c1, 1- sqrt(Tr))).^2);
                a(isnan(a)) = 0;
                for i = 1 : nmole
                    for j = 1 : nmole
                        if i == j
                            ...
                            bic{i,j} = log(2) .* (bsxfun(@rdivide, a(:,j), b(j)) ...
                                -2 .* bsxfun(@rdivide, sqrt(bsxfun(@times, a(:,i), a(:,j))), ...
                                b(i) + b(j)) .* (1 - kij{i,j})) ./ bsxfun(@times, R, T);
                        end
                    end
                end
            end
            ...
            if ncomp > nmole
                [Uref, Alp, Ta] = getMoleIonBIP(model, names, nmole);
                for i = 1 : ncomp
                    for j =  nmole+1 : ncomp
                        if i <= nmole
                            bic{i, j} = (Uref(i) + Alp(i).*((1-T./Ta(i)).^2-(1-298.15./Ta(i)).^2)) ./ T;
                            bic{j, i} = bic{i, j};
                        else
                            bic{i,j}=zeros(nc,1);
                        end
        ...
        function [A_L, A_V, B_L, B_V, acti_L, acti_V] = getMixtureFugacityCoefficients(model, T, x, y, bic, a)
            % Calculate intermediate values for fugacity computation
            [A_L, B_L, acti_L] = model.getPhaseMixCoefficients(x, T, bic, a);
            [A_V, B_V, acti_V] = model.getPhaseMixCoefficients(y, T, bic, a);
        end

        function [Z_L, Z_V, f_L, f_V] = getCompressibilityAndFugacity(model, P, T, x, y, z, Z_L, Z_V, varargin)
            [bic, a, Tr] = model.getBic(T, iscell(x));
            [A_L, A_V, B_L, B_V, acti_L, acti_V] = model.getMixtureFugacityCoefficients(T, x, y, bic, a);
            R = 8.314462618;
            b = model.ECPACompositionalMixture.b;
            epsilonAB = model.ECPACompositionalMixture.epsilonAB;
            betaAB = model.ECPACompositionalMixture.betaAB;
            P_L = P; P_V = P;
            if isempty(Z_L)
                [~, v_L, XA_L, XC_L] = model.computeCompressibilityZ(P_L, T, x, A_L, B_L, a, acti_L, Tr, true);
            else
                v_L = Z_L .* R .* value(T) ./ value(P_L);
                [XA_L, XC_L] = model.pressureA(value(T), value(x), value(B_L), epsilonAB, betaAB, b, v_L, value(Tr));
            end
            if isempty(Z_V)
                [~, v_V, XA_V ,XC_V] = model.computeCompressibilityZ(P_V, T, y, A_V, B_V, a, acti_V, Tr, false);
            else
                v_V = Z_V .* R .* value(T) ./ value(P_V);
                [XA_V, XC_V] = model.pressureA(value(T), value(y), value(B_V), epsilonAB, betaAB, b, v_V, value(Tr));
            end
            ...
            Z_V = P_V .* v_V ./ (R .* T);
            Z_L = P_L .* v_L ./ (R .* T);
            f_L = model.computeFugacity(T, P_L, x, A_L, B_L, a, v_L, acti_L, XA_L, XC_L, Tr);
            f_V = model.computeFugacity(T, P_V, y, A_V, B_V, a, v_V, acti_V, XA_V, XC_V, Tr);
        end

        function [A, B, acti] = getPhaseMixCoefficients(model, x, T, bic, a)
            R = 8.314462618;
            b = model.ECPACompositionalMixture.b;
            [B, C] = deal(0);
            if iscell(x)
                ...
                G = b(:) ./ b;
                nc = numel(T);
                st = cell(1, ncomp);s = cell(1, ncomp);
                S = cell(1, ncomp);acti= cell(1, ncomp);
                for i=1:ncomp
                    st{i}=zeros(nc,1);s{i}=zeros(nc,1);
                    for j=1:ncomp
                        st{i} = st{i} + x{j} .* bic{j,i} .* G(j,i);
                        s{i} = s{i} + x{j} .* G(j,i);
                    end
                    S{i} = st{i} ./ s{i};
                end
                for i=1:ncomp
                    gt=0;
                    for j=1:ncomp
                        gt = gt + x{j} .* G(i,j) ./s{j}.*(bic{i,j}-S{j});
                    end
                    acti{i} =S{i} + gt;
                end
            else
                ...
                for i = 1 : ncomp
                    Gx = 0;
                    for j = 1 : ncomp
                        Gx = Gx + bsxfun(@times, bsxfun(@times,x(:,j), b(j)), bic{j,i});
                    end
                    G = G + x(:,i) .* Gx;
                end
                B = sum(bsxfun(@times, x, b), 2);
                G_E = bsxfun(@times, R, T .* G ./ B);
                A = B .* (sum(bsxfun(@rdivide,bsxfun(@times,x, a), b), 2) ...
                    - bsxfun(@rdivide, G_E, log(2)));

                G = bsxfun(@rdivide, b(:), b);
                nc = numel(T);
                for i=1:ncomp
                    st(:,i)=zeros(nc,1);s(:,i)=zeros(nc,1);
                    for j=1:ncomp
                        st(:,i) = st(:,i) + bsxfun(@times, x(:,j) .* bic{j,i}, G(j,i));
                        s(:,i) = s(:,i) + bsxfun(@times, x(:,j), G(j,i));
                    end
                    S(:,i) = st(:,i) ./ s(:,i);
                end
                for i=1:ncomp
                    gt=0;
                    for j=1:ncomp
                        gt = gt + bsxfun(@times, x(:,j), G(i,j)) ./ s(:,j) .* (bic{i,j}-S(:,j));
                    end
                    acti(:,i) = S(:,i) + gt;
                end
            end
        ...
        function [f, LOGphi] = computeFugacity(model, T, P, xy, A, B, a, v_m, acti, XAx, XCx, Tr)
            b = model.ECPACompositionalMixture.b;
            alpha0 = model.ECPACompositionalMixture.alpha0;
            rBorn = model.ECPACompositionalMixture.rborn;
            rBorn = rBorn(~isnan(rBorn));
            ...
            nmole = ncomp - nion;
            nc = numel(T);
            FuSRK = model.srkFu(T, P, A, B, v_m, a, b, acti);
            if ~isempty(mu0)
                FuAsso = model.assocWaterFu(xy, B, v_m, b, XAx, XCx);
                if ~isempty(d)
                    IFP = model.infiniteFrequencyPermittivity(xy, alpha0, v_m);
                    if iscell(xy)
                        g = 3.22 - 0.94 .* Tr{1};
                    else
                        g = 3.22 - 0.94 .* Tr(:,1);
                    ...
                    SP = model.staticPermittivity(T, xy, theta, g, mu0, v_m, IFP);
                    Kappa = model.debyeLength(T, xy, charge, v_m, SP);
                    Sigama = model.sigamaFunction(d, Kappa, iscell(xy));
                    DIFPn = model.derivativeIFPn(IFP, v_m, alpha0, iscell(xy));
                    DSPns = model.derivativeSPns(DIFPn, SP, IFP, T, v_m, g, mu0, theta, iscell(xy));
                    MuDHs = model.chemicalPotentialDHs(T, xy, charge, v_m, Sigama, Kappa, SP, DSPns);
                    ...
                    MuDHi = model.chemicalPotentialDHi(T, xy, charge, v_m, Sigama, Kappa, SP, DSPni, Kai);
        ...
        function [XAxy, XCxy, gxy] = pressureA(model, T, xy, B, epsilonAB, betaAB, b, v_m, Tr)
            nmole = model.getNumberOfMolecules();
            names = model.ECPACompositionalMixture.names;

            rouxy = bsxfun(@rdivide, 1, v_m);
            etaxy = 0.25 * rouxy .* B;
            gxy = bsxfun(@rdivide, 1, 1 - bsxfun(@times, 1.9, etaxy));
            dexy(:,1) = bsxfun(@times, gxy .* (exp(bsxfun(@rdivide, epsilonAB(1), T)) - 1), b(1) * betaAB(1));
            F = model.getPseudoAssociationPara(Tr, {names{1:nmole}});
            for i = 2 : nmole
                dexy(:,i) = dexy(:,1) .* F{i};
            end
            ...
            nc = numel(T);
            active = true(nc, 1);
            rouxy_loc = rouxy(active);
            xy_loc = xy(active, :);
            Xaxy_loc = ones(nc, nmole);
            dexy_loc = dexy(active, :);
            XCxy = ones(nc, 1);
            XAxy = ones(nc, nmole);
            for i = 1 : 1000
                XCxy_loc = bsxfun(@rdivide, 1, (1 + 2 * rouxy_loc .* xy_loc(:,1) .* Xaxy_loc(:,1) .* dexy_loc(:,1) ...
                    + rouxy_loc .* sum(xy_loc(:,2:nmole) .* Xaxy_loc(:,2:nmole) .* dexy_loc(:,2:nmole), 2)));
                XAxy_loc = bsxfun(@rdivide, 1, (1 + bsxfun(@times, 2 * rouxy_loc .* xy_loc(:,1) .* XCxy_loc, dexy_loc)));

                R_norm = sum(abs(XAxy_loc - Xaxy_loc) ./ Xaxy_loc, 2);
                converged = R_norm < 1e-6;
                trivial = isnan(R_norm);
                done = trivial | converged;
                keep = ~done;
                replace = active;
                replace(active) = done;
                XCxy(replace) = XCxy_loc(done);
                XAxy(replace, :) = XAxy_loc(done, :);
                active(active) = ~done;

                if all(done)
                    break
                end
                rouxy_loc = rouxy_loc(keep);
                xy_loc = xy_loc(keep, :);
                Xaxy_loc = XAxy_loc(keep, :);
                dexy_loc = dexy_loc(keep, :);
            end
            if ~all(done)
                warning('Site fraction solver did not converge');
            end
        end

        function  F = getPseudoAssociationPara(model, Tr, names)
            ncomp = numel(names);
            F = cell(1, ncomp);
            for i = 2 : ncomp
                switch(lower(names{i}))
                    case {'carbondioxide', 'co2'}
                        F{i} = 0.4954 .* Tr(:,i) - 0.3818;
                    case {'hydrogensulfide', 'h2s'}
                        F{i} = 0.8341 .* Tr(:,i) - 0.4391;
                    case {'sulfurdioxide', 'so2'}
                        F{i} = 0.2273 .* Tr(:,i) - 0.1084;
                    case {'methane', 'nitrogen', 'oxygen', 'argon', 'ch4', 'n2', 'o2', 'ar'}
                        F{i} = 0;
                end
            end
        end
        ...
        function Fu = srkFu(model, T, P, A, B, v_m, a, b, activity)
            R = 8.314462618;
            if iscell(a)
                ncomp = model.getNumberOfComponents();
                Fu = deal(cell(1, ncomp));
                f1 = 1./(v_m-B) - A./(R.*T.*B.*(v_m+B));
                f2 = - log(P .* (v_m - B) ./ (R .* T));
                f3 = log((v_m+B)./v_m);
                for i = 1:ncomp
                    Fu{i} = f2 + b(i).*f1 - (a{i}./(R.*T.*b(i)) - activity{i}./log(2)).*f3;
                end
            else
                Fu = bsxfun(@times, (bsxfun(@rdivide, 1, (v_m - B)) ...
                    - A ./ (B .* R .* T .* (v_m + B))), b) ...
                    - log(P .* (v_m - B) ./ (R .* T)) ...
                    - bsxfun(@times, (bsxfun(@rdivide, a, bsxfun(@times, b, R .* T)) ...
                    - activity ./ log(2)), log((v_m + B) ./ v_m));
            end
        end

        function Fu = assocWaterFu(model, xy, B, v_m, b, XAx, XCx)
            ncomp = model.getNumberOfComponents();
            nion = model.getNumberOfIons();
            nmole = ncomp - nion;
            if iscell(xy)
                ...
                Fu = assox - 0.5 .* 1.9 .* bsxfun(@rdivide, bsxfun(@times, b, asso), 4 .* v_m - 1.9 .* B);
            end
        end
        ...
        function [XA, XC] = setXDerivatives(model, T, xy, B, epsilonAB, betaAB, b, v, Tr, XA, XC, i)
            nmole = model.getNumberOfMolecules();
            nc = numel(T);
            names = model.ECPACompositionalMixture.names;
            ...
            g = 1 ./(1 - 1.9*eta);
            de = cell(1, nmole);
            de{1} = g .* (exp(epsilonAB(1)./T) - 1).* b(1) .* betaAB(1);
            F = model.getPseudoAssociationPara(Tr, {names{1:nmole}});
            for j = 2 : nmole
                de{j} = de{1} .* F{j};
            end
            ...
            if isDiagonal
                if isempty(XC.jac{i}.diagonal)
                    XC.jac{i}.diagonal = v.jac{i}.diagonal;
                end
                active = true(nc, 1);
                rou_loc = rou(active);
                xy_loc = cellfun(@(x) x(active), xy, 'UniformOutput', false);
                de_loc = cellfun(@(x) x(active), de, 'UniformOutput', false);
                XA_loc = cellfun(@(x) x(active), XA, 'UniformOutput', false);
                XC_loc = XC(active);
                for itea = 1 : 10000
                    for j = 1 : nmole
                        XA_loc{j} = 1./(1 + 2*rou_loc.*xy_loc{1}.*XC_loc.*de_loc{j});
                    end
                    XCm = 1 + 2*rou_loc .* xy_loc{1}.*XA_loc{1}.*de_loc{1};
                    for j = 2 : nmole
                        XCm = XCm + rou_loc.*xy_loc{j}.*XA_loc{j}.*de_loc{j};
                    end
                    XC_nex = 1./ XCm;
                    dXCn = XC_nex.jac{i}.diagonal;
                    dXC = XC_loc.jac{i}.diagonal;
                    if rowMajor
                        nz = all(dXCn, 2);
                        R_norm = max(abs((dXCn(nz,:)-dXC(nz,:))./dXC(nz,:)),[],1)';
                    else
                        nz = all(dXCn, 1);
                        R_norm = max(abs((dXCn(:,nz)-dXC(:,nz))./dXC(:,nz)),[],2);
                    end
                    done = R_norm < 1e-6;
                    if isempty(R_norm)
                        done = active;
                    ...
                    rou_loc = rou_loc(keep);
                    xy_loc = cellfun(@(x) x(keep), xy_loc , 'UniformOutput', false);
                    XC_loc = XC_nex(keep);
                    de_loc = cellfun(@(x) x(keep), de_loc , 'UniformOutput', false);
                end
                if ~all(done)
                    warning('Site fraction derivative solver did not converge');
                end
            else
                if isempty(XC.jac{i})
                    XC.jac{i} = v.jac{i};
                ...
                for itea = 1 : 10000
                    ...
                    XC_nex = 1./ XCm;
                    dXCn = model.getJac(XC_nex,i);
                    dXC = model.getJac(XC_loc,i);
                    R_norm = abs((dXCn-dXC)./dXC);
                    R_zero = dXCn-dXC;
                    converged = R_norm < 1e-6;
                    trivial = R_zero == 0;
                    done = trivial | converged;
                    ...
                    rou_loc = model.keepADI(rou_loc, keep);
                    XC_loc = model.keepADI(XC_nex, keep);
                    for j = 1 : nmole
                        xy_loc{j} = model.keepADI(xy_loc{j}, keep);
                        de_loc{j} = model.keepADI(de_loc{j}, keep);
                    end
                end
                if ~all(done)
                    warning('Site fraction derivative solver did not converge');
